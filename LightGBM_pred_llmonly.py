...
def predict_pipeline():
    ...
    feature_path = './features/weibo_qwen3_features_predict_32.csv'
    output_dir = './save'
    ...
    try:
        ...
        # 2. 合并数据
        merged_df = pd.merge(df_predict, df_features, on=['uid', 'mid'], how='left')

        # 3. 处理特征 (与训练时保持一致)
        content_features = merged_df['content_feature'].str.split(' ', expand=True).astype(float)
        content_features.columns = [f'feature_{i}' for i in range(32)]
        X_pred = pd.concat([merged_df[['uid', 'mid']], content_features], axis=1)
        ...
        features = [f'feature_{i}' for i in range(32)]
