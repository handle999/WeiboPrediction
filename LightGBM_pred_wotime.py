...
def predict_pipeline():
    ...
    try:
        ...
        # remove = ['time_weekday','time_weekend','time_hour','panduan']
        remove = ['length_all','length_chinese','english','non_ch','sharing','auto','interaction','book','mention','vote','lottery','emoji','video','http','stock','app','title','ad','keywords']
        # remove = ['number_in_train','forward_max','comment_max','like_max','forward_min','comment_min','like_min','forward_mean','comment_mean','like_mean','forward_judge','comment_judge','like_judge']
        features = X_pred.columns.drop(['uid', 'mid', 'time'] + remove)
        print(features)
        print(X_pred[features].shape)
        
        pred_forward = np.expm1(forward_model.predict(X_pred[features]))
        pred_comment = np.expm1(comment_model.predict(X_pred[features]))
        pred_like = np.expm1(like_model.predict(X_pred[features]))
